...
// =============== MODULE: Dark/Light Theme Switcher ===============
...
// =============== MODULE: Fixed Header on Scroll ===============
    var headerFixed = function () {
    if ($('body').hasClass('header-fixed')) {
        var nav = $('.header');
        if (nav.length) {
            var isMobile = window.innerWidth <= 550; // hoặc 768 tuỳ breakpoint của bạn
            var headerHeight = nav.outerHeight();
            var injectSpace = null;
            ...
            $(window).on('load scroll', function () {
                ...
                if (isMobile) {
                    nav.addClass('is-fixed is-small'); // luôn luôn cố định + thu nhỏ
                    if (injectSpace) injectSpace.hide();
                } else {
                    if (scrollTop > 100) {
                        nav.addClass('is-fixed');
                        injectSpace && injectSpace.show();
                    } else {
                        nav.removeClass('is-fixed');
                        injectSpace && injectSpace.hide();
                    }

                    if (scrollTop > 200) {
                        nav.addClass('is-small');
                        injectSpace && injectSpace.height(60); // tùy chiều cao nav nhỏ
                    } else {
                        nav.removeClass('is-small');
                        injectSpace && injectSpace.height(headerHeight);
                    }
                }
            });
        }
...
// =============== MODULE: Newsletter AJAX Subscribe ===============
    var ajaxSubscribe = {
        obj: {
...
// =============== INIT: App Bootstrapping on DOM Ready ===============
    $(function () {
        headerFixed();
        mobileNav();
